...
'OK, replace consist
    ...
    If flagGrid = 1 Then
        ...
        FileCopy strRoute & "\Services\" & strSrv, strTempFilesPath & "\" & strSrv
        DoEvents

        flagway = 0    'unicode to ascii
        Call ConvertIt(strTempFilesPath & "\" & strSrv, flagway, strOldCon, strNewCon)

        Kill strRoute & "\Services\" & strSrv
        flagway = 1    'unicode to ascii
        Call ConvertIt(strTempFilesPath & "\" & strSrv, flagway, strOldCon, strNewCon)
        FileCopy strTempFilesPath & "\" & strSrv, strRoute & "\Services\" & strSrv
        Kill strTempFilesPath & "\" & strSrv
...
Private Function ConvertIt(CompleteFilePath As String, flagway As Integer, strOldCon As String, strNewCon As String) As Boolean
    ...
    Dim strStart As String, strEnd As String, xx As Long, x As Long

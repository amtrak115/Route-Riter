...
Dim strTempFilesParent          As String
...
Private WithEvents objSP As cScanPath
Attribute objSP.VB_VarHelpID = -1
...
Private Function CheckDefaultWag()
    ...
    If FileExists(strWag) Then
        ...
        If x > 0 Then
            ...
            If strTemp <> "0.1" Then
                Select Case MsgBox("The Current Instance of MSTS Trainset does not appear to include the modified DEFAULT.wag file" & vbCrLf & "required for use with these Couplers - Do you wish to install it?", vbYesNo Or vbExclamation Or vbDefaultButton1, App.Title)
    ...
    Else
        Select Case MsgBox("The Current Instance of MSTS Trainset does not appear to include the modified DEFAULT.wag file" & vbCrLf & "required for use with these Couplers - Do you wish to install it?", vbYesNo Or vbExclamation Or vbDefaultButton1, App.Title)
        Case vbYes
            If FileExists(App.Path & "/Default/Default.wag") Then
                FileCopy App.Path & "/Default/Default.wag", strWag
            Else
                Call MsgBox("File " & App.Path & "/Default/Default.wag is missing.", vbCritical)
            End If
        Case vbNo
            Exit Function
        End Select
...
Private Function ReplaceAirBrakes(MyString As String, _
                                  strBrakeFile As String, _
                                  strFP As String) As Boolean
...
Private Function ReplaceBrakes(MyString As String, _
                               strBrakeFile As String, _
                               strFP As String) As Boolean
...
Private Function WriteUniFile(CompleteFilePath As String, MyString As String) As String
    ...
    Exit Function
Errtrap:
    Call MsgBox(Err.Description & " occurred in WriteUniFile while processing" & vbCrLf & "file " & CompleteFilePath, vbExclamation, App.Title)
End Function
Sub EvalExpression(E As String)
    ...
    Dim pp3   As Integer, ppf As String
...
Function GetValues(ByVal ee, ByVal ppe, ByVal vlist) As String
    Dim pp1 As Integer, pp2 As Integer, vflag1 As Boolean, vflag2 As Boolean
    Dim A   As String, VLeft As String, VRight As String
    Dim ppp As Long, eee As String, xxx As String
...
End Function
Private Function ReplaceCouplings_Auto(MyString As String, _
                                       sLen As Single, _
...
End Function
Private Function ReplaceCouplings(MyString As String, _
                                  strCouplingFile As String) As Boolean
...
Private Sub Command1_Click(Index As Integer)
    ...
    Case 0 'steam locos
        ...
        'result = DirDiver(FirstPath, DirCount, "", intType)
        objSP.Filter = "*.eng"
        objSP.StartScan FirstPath, True, False, True, False
    Case 1 'diesel loco
...
Private Sub Command10_Click()
    ...
    Dim X1              As Single, X2 As Single, y1 As Single, y2 As Single, z1 As Single, z2 As Single, x3 As Single, sngMass As Single
    ...
    Dim strShapePath    As String, A$, Y As Long
    ...
    If Not DirExists(strTempFilesPath) Then
        MkDir strTempFilesPath
    End If
    Kill strTempFilesPath & "\*.*"
    DoEvents
    ...
    For i = 0 To List1.ListCount - 1
        ...
        If List1.Selected(i) = True Then
            ...
            strBatText = ChrW$(34) & strTempFilesParent & "\sviewRR4.exe" & ChrW$(34) & " " & ChrW$(34) & strPicView & ChrW$(34)
            ChDrive Left$(strTempFilesParent, 1)
            ChDir strTempFilesParent
            Call ShellAndWait(strBatText, True, vbNormalFocus)
            DoEvents
            NewFile = FreeFile
            Open strTempFilesPath & "\tempsize.txt" For Input As #NewFile
            Input #NewFile, A$
            X1 = CSng(A$)
            Input #NewFile, A$
            y1 = CSng(A$)
...
Errtrap:
    If Err.Number = 53 Then
        ...
        Resume Next
    End If
    Select Case MsgBox("An error " & Err.Number & " " & Err.Description & " occurred in subroutine 'Command10_CLick' processing" & vbCrLf & fullpath$, vbRetryCancel Or vbExclamation Or vbDefaultButton1, App.Title)
    Case vbRetry
...
Private Sub Command11_Click()
    ...
    Dim strDerail    As String, strDerailBuf As String
...
CarryON:
    ...
    strLength = Text1(0)
    If strLength <> vbNullString Then
        x = InStr(strLength, "m")
...
Private Sub Command14_Click()
    ...
    Call MsgBox("When the Shape File Viewer screen appears, the measuring tool may not appear correctly aligned, (a single" & vbCrLf & "layer of lines), if this is so, select the Tools\Orthagonal View from the Menu to correct it (you may need to click twice).", vbInformation, App.Title)
...
Private Sub Command16_Click()
    ...
    Dim strShapePath    As String, A$
    Dim Y               As Long, strStart As String, strEnd As String
    ...
    CDL1.Filename = ""
    If flagType = 4 Then
        Select Case MsgBox("If length of wagon is less than 11.89m (39 ft) then use the" & vbCrLf & "'Short Wagon' coupler instead of your selected coupler?", vbYesNo Or vbExclamation Or vbDefaultButton1, App.Title)
    ...
    For i = 0 To List1.ListCount - 1
        If List1.Selected(i) = True Then
            ...
            ChDrive Left$(strTempFilesParent, 1)
            ChDir strTempFilesParent
            'create files at \TempFiles\tempsize.txt
            Call ShellAndWait(strBatText, True, vbNormalFocus)
            DoEvents
            NewFile = FreeFile
            Open strTempFilesPath & "\tempsize.txt" For Input As #NewFile
...
Private Sub Command17_Click()
    ...
    DoEvents
    If flagType = 2 Then
        Select Case MsgBox("You are modifying the Brake settings on Diesel Locomotives, do you wish the program to automatically" & vbCrLf & "select the Brake Values based on the mass of the selected locomotives?", vbYesNo Or vbExclamation Or vbDefaultButton1, App.Title)
    ...
    End If
    If flagType = 4 Then
        Select Case MsgBox("You are modifying the Brake settings on Freight Wagons, do you wish the program to automatically" & vbCrLf & "select the Brake Values based on the mass of the selected wagons?", vbYesNo Or vbExclamation Or vbDefaultButton1, App.Title)
    ...
    End If
    If flagType = 5 Then
        Select Case MsgBox("You are modifying the Brake settings on Passenger Cars, do you wish the program to automatically" & vbCrLf & "select the Brake Values for you?", vbYesNo Or vbExclamation Or vbDefaultButton1, App.Title)
...
CarryON:
    ...
    Exit Sub
Errtrap:
    Call MsgBox("An error - " & Err.Description & " occurred in" & List1.List(i) & vbCrLf & "File not modified." & vbCrLf & "", vbExclamation, App.Title)
    GoTo CarryON

End Sub

Private Sub Command19_Click()
    'Correct couplers automatically
    Dim i                  As Integer, x As Integer, xx As Integer, j As Integer
    Dim strBatText         As String, strTemp As String
    Dim strShape           As String, strWagName As String
    Dim NewFile            As Integer, intSteam As Integer
...
Private Function DirDiver(NewPath As String, _
                          DirCount As Integer, _
                          Backup As String, _
                          intType As Integer) As Integer
...
SkipThis:
    ...
    Exit Function
DirDriverHandler:
    If Err.Number = 7 Then             ' If Out of Memory error occurs, assume the list box just got full.
...
Private Sub Command20_Click()
    Dim i As Integer
    On Error GoTo Errtrap
    Select Case MsgBox("Please confirm you really wish to DELETE all selected files." & vbCrLf & "This operation is irreversible.", vbYesNo Or vbExclamation Or vbDefaultButton1, App.Title)
...
CarryON:
    ...
    Exit Sub
Errtrap:
    If Err.Number = 75 Then GoTo CarryON
End Sub
...
Brakes1:
    If booIron = False Then
        Select Case MsgBox("You are about to change the brake settings for COMPOSITION brakes in" & vbCrLf & "the selected rolling-stock. Do you really wish to do this?", vbYesNo Or vbExclamation Or vbDefaultButton1, App.Title)
    ...
    ElseIf booIron = True Then
        Select Case MsgBox("You are about to change the brake settings for CAST-IRON brakes in" & vbCrLf & "the selected rolling-stock. Do you really wish to do this?", vbYesNo Or vbExclamation Or vbDefaultButton1, App.Title)
...
Private Sub Command23_Click()
    ...
    End If
Brakes1:
    Select Case MsgBox("You are about to change the brakes to Pro_24RL values in the selected stock." & vbCrLf & "Do you really wish to do this?", vbYesNo Or vbExclamation Or vbDefaultButton1, App.Title)
...
Private Sub Command24_Click()
    ...
    End If
Brakes1:
    Select Case MsgBox("You are about to change the Throttle-Brake percentage values in the selected Locomotive(s)." & vbCrLf & "Do you really wish to do this?", vbYesNo Or vbExclamation Or vbDefaultButton1, App.Title)
    ...
    For i = 0 To List1.ListCount - 1
        If List1.Selected(i) = True Then
            ...
            If Right$(List1.List(i), 4) <> ".eng" Then
                Call MsgBox(List1.List(i) & vbCrLf & "Is not an .eng file and has been ignored.", vbExclamation, App.Title)
                GoTo CarryON
            End If
            ...
            If yy < Y Then
                Call MsgBox("Unable to fix " & strBrake & vbCrLf & "Format unsuitable for automatic update, please fix manually.", vbExclamation, App.Title)
...
Private Sub Command25_Click()
    ...
    Dim strLoco  As String, j As Integer

    Text4.Text = ""
    dlgFriction.Show 1
    DoEvents
    If Text4.Text = "" Then Exit Sub
    Select Case MsgBox("You are about to change the Friction values in the selected Wagons." & vbCrLf & "Do you really wish to do this?", vbYesNo Or vbExclamation Or vbDefaultButton1, App.Title)
    ...
    For i = 0 To List1.ListCount - 1
        If List1.Selected(i) = True Then
            ...
            If Right$(List1.List(i), 4) <> ".wag" Then
                Call MsgBox(List1.List(i) & vbCrLf & "Is not a .wag file and has been ignored.", vbExclamation, App.Title)
                GoTo CarryON
            End If
            ...
            If x = 0 Then
                Call MsgBox("No 'Friction (' entry was found in" & vbCrLf & fullpath$, vbExclamation, App.Title)
...
Private Sub Command26_Click()
    ...
    If Not FileExists(App.Path & "\Formulae.txt") Then
        Call MsgBox("No Formulae.txt file found in your Route_Riter folder?" & vbCrLf & "", vbExclamation, App.Title)
...
Private Sub Command27_Click()
    ...
    Dim strShape     As String, strWagName As String
    Dim NewFile      As Integer, intSteam As Integer
    Dim X1           As Single, X2 As Single, y1 As Single, y2 As Single, z1 As Single, z2 As Single, x3 As Single
...
Private Sub Command28_Click()
    ...
    Select Case MsgBox("You are about to Enable Shunting on the selected Wagons." & vbCrLf & "Do you really wish to do this? It has no effect on their performance", vbYesNo Or vbExclamation Or vbDefaultButton1, App.Title)
    ...
    For i = 0 To List1.ListCount - 1
        If List1.Selected(i) = True Then
            ...
            If x = 0 Then
                strReport = strReport & "Wagon has not been converted for Shunting" & vbCrLf & fullpath$ & vbCrLf
                GoTo CarryON
...
Private Sub Command29_Click()
    ...
    Select Case MsgBox("You are about to Disable Shunting on the selected Wagons." & vbCrLf & "Do you really wish to do this? ", vbYesNo Or vbExclamation Or vbDefaultButton1, App.Title)
...
Private Sub Command3_Click()
    ...
    For i = 1 To intIgnore
        If strKey = EngIgnore(i) Then
            Call MsgBox("The selected KEY is a multi-line key, and can not be edited with this program." & vbCrLf & "Suggest you edit manually.", vbExclamation, App.Title)
...
Private Sub Command30_Click()
    ...
    Select Case MsgBox("You are about to add a ShuntMaxBrakeForce entry to the selected Wagons." & vbCrLf & "Do you really wish to do this? It has no effect on their performance", vbYesNo Or vbExclamation Or vbDefaultButton1, App.Title)
    ...
    For i = 0 To List1.ListCount - 1
        ...
        If x = 0 Then
            ...
            strReport = strReport & "No 'MaxBrakeForce (' entry was found in" & vbCrLf & fullpath$ & vbCrLf
            GoTo CarryON
        End If
        x = InStr(x, MyString, ")")
...
TryAnother3:
    ...
    If Option1(0).value = True And Check1.value = 1 Then
        If strInsert = vbNullString Then
            Call MsgBox("You do not appear to have selected an item to change, but the" & vbCrLf & "Check Box has been selected?", vbExclamation, App.Title)
...
Private Sub Command5_Click()
    ...
    If strValue = vbNullString Then
        Select Case MsgBox("You have not provided a value for this KeyWord?" & vbCrLf & "If this Key does not have parameters, then press OK, else Cancel.", vbOKCancel Or vbExclamation Or vbDefaultButton1, "No Value Given?")
...
Private Sub Command8_Click()
    ...
    For i = List1.ListCount - 1 To 0 Step -1
        List1.RemoveItem i
    Next i
    lblCount(0).Caption = List1.SelCount
    lblCount(1).Caption = List1.SelCount
End Sub
...
Private Sub EW_DirMatch(Directory As String, Path As String)
...
Private Sub Form_Load()
    ...
    If Not DirExists(strTempFilesPath) Then
        ...
        DoEvents
    End If
    strTempFilesParent = Left$(strTempFilesPath, InStrRev(strTempFilesPath, "\") - 1)
    If FileExists(App.Path & "\sviewRR4.exe") Then
        FileCopy App.Path & "\sviewRR4.exe", strTempFilesParent & "\sviewRR4.exe"
    End If
Label1:
    If Not FileExists(App.Path & "\enginekeys.txt") Then
        Call MsgBox("The file 'EngineKeys.txt' is missing from the Route_Riter folder," & vbCrLf & "without it this option will not run.", vbExclamation, App.Title)
        Exit Sub
    End If
    If Not FileExists(App.Path & "\engineignore.txt") Then
        Call MsgBox("The file 'EngineIgnore.txt' is missing from the Route_Riter folder," & vbCrLf & "without it this option will not run.", vbExclamation, App.Title)
        Exit Sub
    End If
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    Kill strTempFilesParent & "\sviewRR4.exe"
    Kill strTempFilesParent & "\TC_Displaybase.s"
    Exit Sub
Handler:
    If Err.Number = 53 Then
        Resume Next
...
Private Sub objSP_FileMatch(Filename As String, Path As String)
    ...
    End If
    If flagType <> 9 Then
        Call GetFileType(Path & "\" & Filename, intFile)
    End If
    If flagType = 9 Or flagType = 8 Or flagType = intFile Then
        x = InStrRev(Path, "\")
        strFolder = Mid$(Path, x + 1)
        Entry = strFolder & "\" & Filename
        List1.AddItem Entry
        lblCount(0).Caption = Str(Val(lblCount(0).Caption) + 1)
    End If
End Sub

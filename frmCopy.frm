...
Private Sub CopyRoute()
    ...
    If strBatFile <> vbNullString Then
        intMini = FreeFile
        Open strTempFilesPath & "\mini.bat" For Output As #intMini
        Print #intMini, strBatFile
        Close #intMini
        ChDrive Left$(strTempFilesPath, 1)
        ChDir strTempFilesPath
        DoEvents
...
Private Sub DoDeComp2(strFile As String, strFPath As String, strSparePath As String)
    ...
    strBatText = "java -cp " & ChrW$(34) & strTsUtilPath & ChrW$(34) & " TSUtil -l" & ChrW$(34) & strReportsPath & "\" & RouteName & "_decomp.log" & ChrW$(34) & "  fmgr " & strSuffix & " -e -n" & ChrW$(34) & strFile & ChrW$(34) & " " & ChrW$(34) & strFPath & ChrW$(34) & " " & ChrW$(34) & strSparePath & ChrW$(34)
...
Private Sub GetRef()
    ...
    Dim x As Long, Y As Long, yy As Long
...
'Select target route
    ...
    If Index = 0 Then
        ...
        If booMini = True And Right$(DestRoutePath, 6) <> "Routes" Then
            Call MsgBox("The selected folder is not a 'Routes' folder." & vbCrLf & "Select the correct mini-routes 'Routes' folder and try again.", vbExclamation, App.Title)
            Exit Sub
        End If
    End If
    If Index = 1 Then
        ...
        If booMini = True Then
            Call CopyRoute
            DoEvents
            Call MsgBox("Your Route has now been copied to your Mini-Route folder. Now:-" & vbCrLf & "1. Go to the Mini-Route folder in the Left-hand window and double clicking on Train Simulator" & vbCrLf & "2. Go to the Files\MSTS Path menu and click on Select to set your Mini-Route as the Default Path" & vbCrLf & "3. Select your Mini-Route and click the Confirm Route button" & vbCrLf & "4. Exit Route_Riter, then restart it to reset everything." & vbCrLf & "5. Click on the Mini-Route Get Stock button to transfer the rolling stock across." & vbCrLf & "", vbInformation, "Setting up a Mini-Route")
            booMini = False
            Exit Sub
        End If
        frmUtils.Text1(0) = "*.s"
    ...
    If Index = 2 Then
        ...
        'copy selected shapes to temporary directory
        cursouind = 0
        For i = 0 To frmUtils.File1(cursouind).ListCount - 1
            If frmUtils.File1(cursouind).Selected(i) Then

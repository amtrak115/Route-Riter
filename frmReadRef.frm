...
Public Sub FindMinMax(ByRef dArray() As Double, _
                      ByRef dLowVal As Double, _
                      ByRef dHighVal As Double, _
                      MinIndex As Long, _
                      maxIndex As Long)
...
Private Sub Command2_Click()
    ...
    Dim strSetupFilesPath As String
    ...
    For i = 0 To RefGrid.Rows - 1
        ...
        DoEvents
    Next i
    strSetupFilesPath = SpecialFolder(SpecialFolder_LocalAppData) & "\Route_Riter\SetupFiles"
    Filpath1$ = strSetupFilesPath & "\"
    If FileExists(Filpath1$ & "TempRef.ref") Then
        Kill Filpath1$ & "TempRef.ref"
        DoEvents
    End If
    FileCopy App.Path & "\reffilestart.txt", Filpath1$ & "TempRef.ref"
    NewFile = FreeFile
...
Private Sub Command6_Click()
    ...
    Select Case MsgBox("It is possible to have two items using the same .s file so take care with this option." & vbCrLf & "Click OK if you wish to continue", vbOKCancel Or vbExclamation Or vbDefaultButton1, "Warning !!")
...
Public Function RemD2(ByRef rArray() As Variant, xArray() As Variant) As Variant
    Dim ii As Long, jj As Long, high As Long
...
Private Sub Form_Resize()
    ...
    Text1.Left = Command9.Left + Command9.width + 300

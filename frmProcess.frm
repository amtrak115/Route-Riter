...
Private Sub ParseH(strText As String, strFlag As String, strTemp As String)
    ...
    Exit Sub
Errtrap:
    Call MsgBox("Error " & Err.Number & " " & Err.Description & " occurred while running ParseH.", vbExclamation, App.Title)
...
Private Sub ParseV(strV As String)
    ...
    If x = 0 Then
        Y = InStr(strV, ":")
        If Y = 0 Then
            Call MsgBox("Invalid format in the 'Manually assign shapes box' must be Old number:New number etc" & vbCrLf & "e.g. 100:137 101:138 etc", vbExclamation, App.Title)
...
Private Sub Button2_Click()
    ...
    Dim strWorldOptions As String
    Dim strMSTSPath     As String
    ...
    Case 28
        If Checkbox6.value = 1 Then
            Textbox5.Text = strWorldOptions & " world -sx " & ChrW$(34) & strSelectedPath & ChrW$(34)
        ElseIf Checkbox7.value = 1 Then
            Textbox5.Text = strWorldOptions & " world -mx " & ChrW$(34) & "-q" & strSelectedPath & "\WORLD" & ChrW$(34) & " " & ChrW$(34) & strSelectedPath & ChrW$(34)
            'Textbox5.Text = strWorldOptions & " world -mx " & ChrW$(34) & strSelectedPath & ChrW$(34)
        End If
        Label9.Visible = True
    ...
    Exit Sub
Errtrap:
    Call MsgBox("Error " & Err.Number & " " & Err.Description & " occurred while processing this option.", vbExclamation, App.Title)
...
Private Sub Command1_Click()
    Dim strTsUtilPath As String
    
    strTsUtilPath = GetSetting("Route_Riter6", "TsUtil", "Path", "")
    ChDrive Left$(App.Path, 1)
    ''ChDir App.Path & "\TSUtil"
    strBatText = Textbox5.Text
    strBatText = "java -cp " & ChrW$(34) & strTsUtilPath & ChrW$(34) & Mid$(strBatText, 5)
...
Private Sub Form_Load()
    ...
    strReportsPath = SpecialFolder(SpecialFolder_LocalAppData) & "\Route_Riter\Reports"
    If Not DirExists(strReportsPath) Then
        MkDir strReportsPath
    End If
    Textbox4.Text = strReportsPath
    ...
    Select Case TSOption
    Case 1
        Label4.Caption = "The function 'wunc' is used to convert ALL world-files to 'uncompressed text'using ffeditc_unicode.exe" & vbCrLf & "No other options are applicable. The uncompressed files will be found in the World folder (suggest you should use the similar option in Route_Riter)"
                
        strOptionCode = "wunc"
    Case 2
        Label4.Caption = "The function 'wcmp' is used to convert ALL world-files to 'compressed binary'using ffeditc_unicode.exe" & vbCrLf & "No other options are applicable. The uncompressed files will be found in the World folder (suggest you should use the similar option in Route_Riter)"
                
        strOptionCode = "wcmp"
    Case 3
        Label4.Caption = "The function 'wcmpo' is used to convert world-files to 'compressed binary' if they are currently uncompressed Text files (otherwise" & vbCrLf & "they are unchanged) using ffeditc_unicode.exe. No other options are applicable. " & vbCrLf & "The uncompressed files will be found in the World folder (suggest you should use the similar option in Route_Riter)"
        strOptionCode = "wcmpo"
    Case 4
        Label4.Caption = "The function 'cmkr' converts '.coo' files to a MSTS .mkr file." & vbCrLf & "e.g. c:\Test.coo is converted to c:\Test.mkr"
        Textbox6.Visible = True
        Button4.Visible = True
        strOptionCode = "cmkr"
    Case 5
        Label4.Caption = "The function 'fmgr' is used to compress/expand .s/.t/.w files or groups of files, including all files in a folder. " & vbCrLf & "This function only works on one file type at a time. See Tsutils_en.txt for full details as to how you can use either" & vbCrLf & "'Regular Expressions' or Windows wild-cards to select those batches of files you wish to process."
        Label7.Visible = True
        Label5.Visible = True
        Textbox6.Visible = True
        Textbox7.Visible = True
    ...
    Case 6
        Label4.Caption = "The function 'chgdb' makes it possible to adapt the track/road database for changes" & vbCrLf & "in the Global\tsection.dat file, this function may be used if a rebuild of .tdb has failed"
        ...
        strOptionCode = "chgdb"
    Case 7
        Label4.Caption = "The function 'dcpy' compares NewRoute with OldRoute and copies any Files/Folders which" & vbCrLf & "have been changed into a third folder. This can be used to provide a patch to update Old to New"
        Checkbox6.Visible = True
        Checkbox7.Visible = True
    ...
    Case 8
        Label4.Caption = "The function 'shift' shifts ranges of ShapeID and/or Section-IDs within a tsection.dat" & vbCrLf & "file. e.g. values 0 400 2000 0 400 2000 moves all shapes and sections between 0 and 400" & vbCrLf & "to positions starting at 2000"
        Checkbox6.Visible = False
        Checkbox7.Visible = True
        Checkbox8.Visible = True
        Checkbox9.Visible = True
        ...
        strOptionCode = "shift"
    Case 9
        Label4.Caption = "The function 'filter' checks all marked tiles in the Route Geometry for completeness." & vbCrLf & "If no .t file exists, the tile is unmarked. If the -w flag is selected, the tile is also" & vbCrLf & "unmarked if no corresponding .w file exists. Both the Tile and Lo_Tile folders are checked"
    ...
    Case 10
        Label4.Caption = "The function 'cmpw' compresses .w files using Martyn Griffin's 'Comp' program." & vbCrLf & "This program must be installed in the 'Utils\Comp' folder for the current MSTS instance."
        strOptionCode = "cmpw"
        ...
        End If
    Case 11
        Label4.Caption = "The function 'tsconv' modifies ALL routes in the current MSTS instance for use with a" & vbCrLf & "new tsection.dat file (see 'cvrt' which only modifies a single route). The original tsection.dat" & vbCrLf & "must be placed in the Routes folder. Alternatively if a route has used a different tsection.dat" & vbCrLf & "This may be placed within that route's folder and named $tsection.dat"
    ...
    Case 12
        Label4.Caption = "The function 'ichk' checks the syntax of the global and local tsection.dat files and" & vbCrLf & "whether references between road/track databases and world files are correct. It also checks" & vbCrLf & "if duplicate UiD identifications are used and if these may be changed. However NO changes are made."
        ...
        strOptionCode = "ichk"
    Case 13
        Label4.Caption = "The function 'rendb' can repair references within track and road databases following" & vbCrLf & "manual deletion of defective Track-Nodes. Gaps are removed etc. See TSUtil_en.txt for full details"
        Checkbox6.Visible = True
        Checkbox7.Visible = True
        Checkbox8.Visible = True
        Checkbox9.Visible = True
        ...
        strOptionCode = "rendb"
    Case 14
        Label4.Caption = "The function 'move' shifts a complete route by the defined number of squares in the X" & vbCrLf & "and/or Z directions."
        Checkbox6.Visible = True
        Checkbox6.value = 1
        Checkbox8.Visible = True
        ...
        strOptionCode = "move"
    Case 15
        Label4.Caption = "The function 'chkup' checks two versions of tsection.dat (both must be in same folder)" & vbCrLf & "to see if the newer one is compatible with the older version." & vbCrLf & "The shape/section definitions may have been altered between the two files."
        Label7.Visible = True
        Textbox6.Visible = True
        ...
        strOptionCode = "chkup"
    Case 16
        Label4.Caption = "The function 'adjh' adjusts the altitude of the route. This adjustment figure which can" & vbCrLf & "be negative is added to all height values in the route definition."
        Checkbox6.Visible = True
        Checkbox6.value = 1
        Checkbox6.Caption = "Write .t files in 'reduced format'"
    ...
    Case 17
        Label4.Caption = "The function 'cmp' will compress all .s/.t/.w files which are in uncompressed text" & vbCrLf & "format to uncompressed binary. With the -c flag, the .s/.w files are then compressed to " & vbCrLf & "compressed binary (.t files cannot be compressed further)"
        Checkbox6.Visible = True
        Checkbox7.Visible = True
        Checkbox8.Visible = True
        Checkbox9.Visible = True
    ...
    Case 18
        ...
        strOptionCode = "unc"
    Case 19
        Label4.Caption = "The function 'merge' creates a common, merged, route from two route definitions." & vbCrLf & "See the TsUtils_en.txt file for restrictions/limitations of this option"
        ...
        strOptionCode = "merge"
    Case 20
        Label4.Caption = "The function 'clrdb' analyzes track- and road-database and deletes all definition-chains" & vbCrLf & "(TrEndNode-TrVectorNode/TrJunctionNode - TrEndNode), which are no longer used. "
        Checkbox6.Visible = True
        Checkbox8.Visible = True
        Checkbox9.Visible = True
    ...
    Case 21
        ...
        strOptionCode = "zusi"
    Case 22
        Label4.Caption = "The function 'shftdyn' is used to shift the range of dynamic track definitions in the" & vbCrLf & "local tsection.dat to their correct position as defined in the Global\tsection.dat"
        Checkbox6.Visible = True
        Checkbox7.Visible = True
        Checkbox8.Visible = True
    ...
    Case 24
        Label4.Caption = "The function 'cvrt' is mainly used to convert a route-definition for use with a new tsection.dat" & vbCrLf & "file (similar to the Horace utility). The original tsection.dat file must also be available." & vbCrLf & "See  TsUtil_en.txt for full details of possible commands including use to produce a 'Hidewire' functionality."
        ...
        CvrtFlag = 1
    Case 25
        Label4.Caption = "The function 'cvrt' is mainly used to convert a route-definition for use with a new tsection.dat" & vbCrLf & "However, 'cvrt' is multifunctional, so use this option to fix invalid SoundSource positions in a route"
        strOptionCode = "cvrt"
        CvrtFlag = 2
    Case 26
        Label4.Caption = "The function 'cvrt' may also be used to give the functionality of the 'Hidewire' utility" & vbCrLf & "See  TsUtil_en.txt for full details of using TsUtils to build a route with some of the overhead wires hidden." & vbCrLf & "To use this, enter, for example 2:A:Xt in the top box - this results in all TrkObjs with a StaticdetailLevel of 2 (may be 1, 2 or 3) being" & vbCrLf & "converted to their static representation. If the filename starts with 'A' it is replaced with 'Xt', e.g. (A1t10mstrt.s -> Xt1t10mstrt.s)"
        ...
        CvrtFlag = 3
    Case 28
        Label4.Caption = "The function world can split and merge static objects from world files."
        Checkbox6.Caption = "Split static objects from world files"
        Checkbox7.Caption = "Merge static objects into world files"
        Checkbox6.Visible = True
        Checkbox7.Visible = True
        strOptionCode = "world"
    End Select

...
Dim objShape As sfModel
Dim objAnim As sfModel
...
Private Sub GetWagPaths(strPix As String, strEP As String, strEN As String, sMass As Single)
    ...
    Dim MyString As String, xx As Long, strTemp2 As String
...
Private Sub GetEngDetail(strEng As String, strCoupler As String, strPower As String, strMass As String, strLen As String, strName As String)
    ...
    Dim strTemp As String, j As Long, jj As Long
...
Private Sub MakeThumbNail(strModel2 As String, strEW As String)
    ...
    ChDir App.Path
    Call CreateThumbnail(strShapePath & strShape, strShapePath & strAnim)
...
Private Sub Form_Resize()
    ...
    If Not objShape Is Nothing Then objShape.ResetState
    If Not objAnim Is Nothing Then objAnim.ResetState
...
Private Sub sfCanvas1_Paint()
    If Not sfCanvas1.d3dd Is Nothing Then showModel
End Sub
Private Sub showModel()
    ...
    If sfCanvas1.BeginScene Then
        ' show the model
        objShape.showModel
        If Not objAnim Is Nothing Then objAnim.showModel
        ' signal end and display the model in the scene
        sfCanvas1.EndScene
    End If
End Sub
Public Sub CreateThumbnail(ByVal strShapePath, ByVal strAnimPath)
    ...
    sfCanvas1.BackColor = &HFFFFFF
    sfCanvas1.time = 600
    sfCanvas1.latitude = 55
    Set objShape = Nothing
    Set objAnim = Nothing
    Set objShape = New sfModel
    Set objShape.d3dd = sfCanvas1.d3dd
    objShape.Filename = strShapePath
    
    ' set up identity matrix
    D3DXMatrixIdentity mw
    ' create rotation matrix about the vertical axis by 180 degrees
    D3DXMatrixRotationY mt, 3.1415926
    ' multiply the two together
    D3DXMatrixMultiply mw, mt, mw
    objShape.setMatrix mw
    
    ' retrieve the view sphere
    objShape.getSphere x, Y, Z, radius
    ' this moves the camera to look at the centre of the model
    ' from a distance that depends on the model size
    sfCanvas1.setCamera radius * 2, 2, 0, x, Y, Z, 0, 1, 0
    ' just make sure everything has caught up
    DoEvents
    If Not strAnimPath = vbNullString Then
        Set objAnim = New sfModel
        Set objAnim.d3dd = sfCanvas1.d3dd
        objAnim.Filename = strAnimPath
        objAnim.setMatrix mw
    End If
    ' signal start of 3D drawing mode
    If sfCanvas1.BeginScene Then
        ' show the model
        ...
        sfCanvas1.EndScene
    End If
    sfCanvas1.screenshotLocation = strTempFilesPath & "\PIX"
    sfCanvas1.screenshotSaveAsJpg = True
    sfCanvas1.saveImage
    Exit Sub
Handler:
...
'Show in 3D
    ...
    Dim strSViewPath As String
    
    strSViewPath = GetSetting("Decapod", "TS Shape Viewer", "AppPath")
    If strSViewPath <> vbNullString Then
        strBatText = ChrW$(34) & strSViewPath & ChrW$(34) & " " & ChrW$(34) & Label10.Caption & ChrW$(34)
        Call ShellAndWait(strBatText, True, vbNormalFocus)
    Else
        ...
        'NewFile = FreeFile
        'Open strTempFiles2Path & "\doBat.bat" For Output As #NewFile
        'Print #NewFile, strBatText
        'Close #NewFile
        ChDrive Left(strTempFiles2Path, 1)
        ChDir strTempFiles2Path
        DoEvents
...
Private Sub Command7_Click()
    ...
    For ii = 1 To numStock
        ...
        If Not FileExists(strStock) Then
            Call MsgBox(strCF & "\" & strCN & strTemp & " listed in this consist appears to be missing?" & vbCrLf & "Consist could not be loaded.", vbExclamation, App.Title)
            Exit Sub
        End If
...
Private Sub Command8_Click()
    ...
    If conLen > origConLen Then
        Select Case MsgBox("During editing you have increased the length of this consist." & vbCrLf & "It may no longer fit in the position where it stands which would cause" & vbCrLf & "an error in MSTS. Do you still wish to proceed?", vbYesNo Or vbExclamation Or vbDefaultButton1, App.Title)
    ...
    If booIgnore = False Then
        For i = 0 To 6
            ...
            If Label8(i + 8).Caption <> Label8(i + 1).Caption Then
                ...
                Select Case MsgBox("There are non-matching couplers in this consist." & vbCrLf & "Do you wish to continue?", vbYesNo Or vbExclamation Or vbDefaultButton1, App.Title)
...
Private Sub mnuActDel_Click()
    ...
    Dim strTemp As String, x As Integer, Y As Integer, xx As Integer, yy As Integer
...
Private Sub mnuSwap_Click()
    ...
    Dim ii As Integer, x As Integer, Y As Integer, xx As Integer, yy As Integer

...
'Create new mini-route
    ...
    If strBatFile <> vbNullString Then
        x = 6
        NewFile = FreeFile
        Open strTempFilesPath & "\mini.bat" For Output As #NewFile
        ...
        DoEvents
        x = 10
        Call ShellAndWait(strTempFilesPath & "\mini.bat", True, vbNormalFocus)
        x = 11
        DoEvents
    End If
    Close
    x = 12
    Call MsgBox("Your Mini-Route structure has now been set up. Now:" & vbCrLf & "Use the Mini-Route Copy button to copy the required files from your Route folder into the Mini-Route 'Route' folder", vbExclamation, App.Title)
    Exit Sub
Errtrap:
    Select Case MsgBox("An error " & Err & " " & Err.Description & " occurred while Creating a Mini-Route please advise" & vbCrLf & "Support that x=" & str(x), vbRetryCancel Or vbExclamation Or vbDefaultButton1, App.Title)
